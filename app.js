...
async function checkAndCompleteReferrals(userId) {
    try {
        ...
        for (const referral of pendingReferrals) {
            ...
            if (referrer) {
                const previousBalance = referrer.balance;
                referrer.balance += 1.5;
                await referrer.save();
                
                console.log(`🎉 REFERRAL BONUS AWARDED: ${referrer.username} received $1.5 bonus for referring ${user.username}`);
                console.log(`   Referrer balance: $${previousBalance} → $${referrer.balance}`);
                
                // Create a notification for the referrer about the bonus
                const notification = new UserNotification({
                    userId: referrer._id,
                    title: 'Referral Bonus Earned!',
                    message: `Congratulations! You earned $1.5 for successfully referring ${user.username}.`,
                    type: 'referral_bonus',
                    isRead: false
                });
                await notification.save();
                
                console.log(`📧 NOTIFICATION CREATED: Referral bonus notification sent to ${referrer.username}`);
            }
...
const userNotificationSchema = new mongoose.Schema({
  ...
  type: { type: String, enum: ['referral_bonus', 'deposit_confirmed', 'withdrawal_approved', 'task_approved', 'general'], default: 'general' },
